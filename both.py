...
sns.pairplot(data=Train_data[['Item_Identifier', 'Item_Weight', 'Item_Fat_Content', 'Item_Visibility',
...
%matplotlib inline
for col in data.select_dtypes(include=[np.number]):dataset.groupby(['Outlet_Location_Type','Outlet_Size'])['Item_Type'].count().plot.bar()
